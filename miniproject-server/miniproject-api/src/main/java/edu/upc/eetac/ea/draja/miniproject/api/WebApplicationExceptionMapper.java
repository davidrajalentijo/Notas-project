...
import edu.upc.eetac.ea.draja.miniproject.api.model.MiniprojectError;
...
public class WebApplicationExceptionMapper implements
	...
	public Response toResponse(WebApplicationException exception) { //construimos la respuesta
		MiniprojectError error = new MiniprojectError(
		...
		return Response.status(error.getStatus()).entity(error) //crea la respuesta a manopla con el estado y el mensaje que le hemos puesta en esta excepcion
				.type(MediaType.MINIPROJECT_API_ERROR).build(); //status el codigo http de la respuesta, el entity es el body de la respuesto y el tipo es el content type, y el build permite crear la respuesta
	}
